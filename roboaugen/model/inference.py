...
def inference(sampleid, file, distort, random_crop, use_cache, threshold, supports, heatmap):
  ...
  predicted_heatmaps = higher_resolution(updated_query_features)

  predicted_heatmaps = predicted_heatmaps.detach().cpu()
  predicted_heatmaps = F.interpolate(predicted_heatmaps, size=(query.size()[2], query.size()[3]), mode='bilinear')
  predictions = display_heatmap('Predictions', query, predicted_heatmaps, heatmap, threshold)
  if targets is not None:
    targets = display_heatmap('Targets', query, targets, heatmap, threshold)
    spatial_penalty = display_heatmap('Spatial Penalty', query, spatial_penalty, heatmap, threshold)
  ...
  supports = supports.squeeze(0)
  supports = [to_numpy_image(supports[sample_idx]) for sample_idx in range(supports.size()[0])]
  supports = np.hstack(supports)
  cv2.imshow(f'Supports', supports)
